...
def upload_s3(json_path):
    ...
    try:
        ...
        json_url = f"https://{S3_BUCKET}.s3.{S3_REGION}.amazonaws.com/{S3_KEY}"
        print(f"  ✅ JSON 업로드 완료: {json_url}")
    ...
    import glob
    html_files = (
        glob.glob("pipeline_dashboard_*.html") +
        glob.glob("*.html")
    )
    # 가장 최근 수정된 HTML 파일 선택
    html_files = [f for f in html_files if "index" not in f]
    if not html_files:
        print("  ⚠️  HTML 파일 없음 — JSON만 업로드됨")
        return True

    html_path = sorted(html_files, key=lambda f: os.path.getmtime(f))[-1]
    print(f"  HTML 파일: {html_path}")

    # ── HTML에 DATA_URL 자동 주입 ────────────────────
    with open(html_path, "r", encoding="utf-8") as f:
        html_content = f.read()

    # DATA_URL을 S3 JSON URL로 교체
    import re
    new_url_line = f"const DATA_URL = '{json_url}';"
    if "const DATA_URL" in html_content:
        # DATA_URL 교체 (작은따옴표/큰따옴표 모두 처리)
        html_content = re.sub(
            r"const DATA_URL = '[^']*';",
            new_url_line,
            html_content
        )
        if new_url_line not in html_content:
            html_content = re.sub(
                r'const DATA_URL = "[^"]*";',
                new_url_line,
                html_content
            )
        print(f"  ✅ DATA_URL 자동 주입: {json_url}")
    else:
        print("  ⚠️  DATA_URL 변수 없음 — HTML 그대로 업로드")
    ...
    try:
        ...
        html_url = f"https://{S3_BUCKET}.s3.{S3_REGION}.amazonaws.com/index.html"
        print(f"  ✅ HTML 업로드 완료")
        print(f"")
        print(f"  ════════════════════════════════════════")
        print(f"  📊 대시보드 URL:")
